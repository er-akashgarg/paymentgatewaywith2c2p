...
data class TransactionResponse(
    ...
    override fun toString(): String {
        return "dateTime=$dateTime, paidAgent=$paidAgent, approvalCode=$approvalCode," +
                " subMerchantID=$subMerchantID, backendInvoice=$backendInvoice, amt=$amt, eci=$eci" +
                ", bankName=$bankName, tranRef=$tranRef, failReason=$failReason, ippInterestRate=$ippInterestRate" +
                ", pan=$pan, ippPeriod=$ippPeriod, uniqueTransactionCode=$uniqueTransactionCode" +
                ", recurringUniqueID=$recurringUniqueID, raw=$raw, version=$version, hashValue=$hashValue" +
                ", paymentChannel=$paymentChannel, issuerCountry=$issuerCountry, timeStamp=$timeStamp" +
                ", refNumber=$refNumber, merchantID=$merchantID, ippMerchantAbsorbRate=$ippMerchantAbsorbRate" +
                ", userDefined4=$userDefined4, userDefined5=$userDefined5, paidChannel=$paidChannel" +
                ", userDefined2=$userDefined2, subMerchantList=$subMerchantList, userDefined3=$userDefined3" +
                ", ippInterestType=$ippInterestType, userDefined1=$userDefined1, respCode=$respCode" +
                ", status=$status, storeCardUniqueID=$storeCardUniqueID"
    }
}